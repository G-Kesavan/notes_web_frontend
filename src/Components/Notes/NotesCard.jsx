    ...
    <div className= 'notes border-blue-100 border rounded-lg p-4 bg-white hover:shadow-xl transition-all ease-in-outz'>
        <div className='flex justify-between'>
            <div>
                ...
                <span className='noteDate text-blue-950/70'>{moment(date).format('Do MMM YYYY')}</span>
            </div>
            <MdOutlinePushPin 
        ...
        <div className='flex justify-between'>
            <div className='text-blue-950/70'>{tags.map((tag,index)=>(<span key={index}>{` #${tag}`}</span>))}</div>
